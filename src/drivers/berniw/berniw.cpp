...
//static 
int  InitFuncPtBerniw(int index, void *pt);
...
int berniw(tModInfo *modInfo)
{
	...
	for (int i = 0; i < BOTS; i++) {
		...
		modInfo[i].fctInit = InitFuncPtBerniw;		/* init function */
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation * situation)
{
	...
	//fix
	//snprintf(buffer, BUFSIZE, "%sdrivers/berniw/%d/%s", GetLocalDir(), index, trackname);
    //*carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
	*carParmHandle = NULL;

	if (*carParmHandle == NULL) {
		snprintf(buffer, BUFSIZE, "%sdata/drivers/berniw/%d/default.xml", GetLocalDir(), index);
	    *carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
	}

...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation * situation)
{
	...
	switch (situation->_raceType) {
		case RM_TYPE_PRACTICE:
			snprintf(buffer, BUFSIZE, "%sdrivers/lliaw/%d/practice/%s", GetLocalDir(), index, trackname);
			break;
		case RM_TYPE_QUALIF:
			snprintf(buffer, BUFSIZE, "%sdrivers/lliaw/%d/qualifying/%s", GetLocalDir(), index, trackname);
			break;
		case RM_TYPE_RACE:
			snprintf(buffer, BUFSIZE, "%sdrivers/lliaw/%d/race/%s", GetLocalDir(), index, trackname);
			break;
		default:
			break;
	}
	...
	*carParmHandle = NULL;
	...
	if (*carParmHandle == NULL) {
		snprintf(buffer, BUFSIZE, "%sdrivers/lliaw/%d/default.xml", GetLocalDir(), index);
		*carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
    }

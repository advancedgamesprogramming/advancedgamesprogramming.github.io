...
namespace olethros {
	...
	switch (s->_raceType) {
        case RM_TYPE_PRACTICE:
            snprintf(buffer, BUFSIZE, "%sdrivers/olethros/%d/practice/%s", GetLocalDir(), INDEX, trackname);
            break;
        case RM_TYPE_QUALIF:
            snprintf(buffer, BUFSIZE, "%sdrivers/olethros/%d/qualifying/%s", GetLocalDir(), INDEX, trackname);
            break;
        case RM_TYPE_RACE:
            snprintf(buffer, BUFSIZE, "%sdrivers/olethros/%d/race/%s", GetLocalDir(), INDEX, trackname);
            break;
        default:
            break;
	}
	...
	*carParmHandle = NULL;
	...
	if (*carParmHandle == NULL) {
            snprintf(buffer, BUFSIZE, "%sdrivers/olethros/%d/default.xml", GetLocalDir(), INDEX);
            //printf ("LOADING: %s\n", buffer);
            *carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
        } else {
            //printf ("LOADING: %s\n", buffer);

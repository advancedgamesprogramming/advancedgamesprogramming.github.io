...
void Driver::initTrack(tTrack* t, void *carHandle, void **carParmHandle, tSituation *s)
{
	...
	switch (s->_raceType) {
		case RM_TYPE_PRACTICE:
			snprintf(buffer, BUFSIZE, "%sdrivers/bt/%d/practice/%s", GetLocalDir(), INDEX, trackname);
			break;
		case RM_TYPE_QUALIF:
			snprintf(buffer, BUFSIZE, "%sdrivers/bt/%d/qualifying/%s", GetLocalDir(), INDEX, trackname);
			break;
		case RM_TYPE_RACE:
			snprintf(buffer, BUFSIZE, "%sdrivers/bt/%d/race/%s", GetLocalDir(), INDEX, trackname);
			break;
		default:
			break;
	}
	...
	*carParmHandle = NULL;
	...
	if (*carParmHandle == NULL) {
		snprintf(buffer, BUFSIZE, "%sdrivers/bt/%d/default.xml", GetLocalDir(), INDEX);
		*carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
    }

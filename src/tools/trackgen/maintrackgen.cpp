...
void usage(void)
{
    fprintf(stderr, "Terrain generator for tracks $Revision: 1.5.2.7 $ \n");
		...
		case 0:
			switch (option_index) {
				case 0:
					...
					break;
				case 1:
					printf("Terrain generator for tracks $Revision: 1.5.2.7 $ \n");
					exit(0);
					break;
				default:
					usage();
...
static void Generate(void)
{
	...
	snprintf(buf, BUFSIZE, "%sdata/modules/track/%s.%s", GetLibDir (), trackdllname, DLLEXT);
	...
	snprintf(trackdef, BUFSIZE, "data/tracks/%s/%s/%s.xml", TrackCategory, TrackName, TrackName);
	...
	if (!JustCalculate) {
		...
		snprintf(buf2, BUFSIZE, "data/tracks/%s/%s/%s", Track->category, Track->internalname, Track->internalname);

...
#define BRK_OFFSET	0.2
	...
	for (j = 0; j < 4; j++) {
		...
		if (grUseDetailedWheels == DETAILED) {
			...
			if (wheel_dir != 0) {
				snprintf(buf, bufsize, "data;data/wheels/%s", wheel_dir);
				ssgModelPath(buf);
				ssgTexturePath(buf);
			}
...
void grInitShadow(tCarElt *car)
{
	...
	snprintf(buf, BUFSIZE, "data/cars/%s;", car->_carName);
	grFilePath = buf;
...
grInitCar(tCarElt *car)
{
	...
	lg = 0;
	lg += snprintf(grFilePath + lg, BUFSIZE - lg, "data/drivers/%s/%d/%s;", car->_modName, car->_driverIndex, car->_carName);
	lg += snprintf(grFilePath + lg, BUFSIZE - lg, "data/drivers/%s/%d;", car->_modName, car->_driverIndex);
	lg += snprintf(grFilePath + lg, BUFSIZE - lg, "data/drivers/%s/%s;", car->_modName, car->_carName);
	lg += snprintf(grFilePath + lg, BUFSIZE - lg, "data/drivers/%s;", car->_modName);
	lg += snprintf(grFilePath + lg, BUFSIZE - lg, "data/cars/%s", car->_carName);
	...
	//fix
#if 0
	ssgStateSelector* pp = (ssgStateSelector*)grEnvSelector->clone();

	grCarInfo[index].envSelector = pp;// (ssgStateSelector*)grEnvSelector->clone();
	grCarInfo[index].envSelector->ref();
#endif
	/* the base transformation of the car (rotation + translation) */
	ssgTransform* tt = new ssgTransform();
	grCarInfo[index].carTransform = tt;// new ssgTransform;
	...
	snprintf(buf, BUFSIZE, "data;data/cars/%s", car->_carName);
	ssgModelPath(buf);
	snprintf(buf, BUFSIZE, "data;data/drivers/%s/%d;data/drivers/%s;data/cars/%s", car->_modName, car->_driverIndex, car->_modName, car->_carName);
	ssgTexturePath(buf);
...
void grDrawCar(tCarElt *car, tCarElt *curCar, int dispCarFlag, int dispDrvFlag, double curTime, class cGrPerspCamera *curCam)
{
	...
	if (grCarInfo[index].envSelector)
	{

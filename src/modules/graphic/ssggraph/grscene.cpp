...
grLoadScene(tTrack *track)
{
	...
	snprintf(buf, BUFSIZE, "data;data/tracks/%s/%s;data/textures;data/img;.", grTrack->category, grTrack->internalname);
	ssgTexturePath(buf);
	snprintf(buf, BUFSIZE, "data;data/tracks/%s/%s", grTrack->category, grTrack->internalname);
	ssgModelPath(buf);

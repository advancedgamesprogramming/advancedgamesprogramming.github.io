...
ReInit(void)
{
	...
	snprintf(buf, BUFSIZE, "%sdata/modules/track/%s.%s", GetLibDir (), dllname, DLLEXT);
#if 0
	...
	//extern "C" int
	//	track(tModInfo *modInfo);
	extern int trackInit(int /* index */, void *pt);
	trackInit(0, &ReInfo->_reTrackItf);
	...
	extern int graphInit(int /* idx */, void *pt);
	
	graphInit(0, &ReInfo->_reGraphicItf);
...
reRegisterRaceman(tFList *racemanCur)
{
	...
	snprintf(buf, BUFSIZE, "%sdata/config/raceman/%s", GetLocalDir(), racemanCur->name);
...
void ReAddRacemanListButton(void *menuHandle)
{
	...
	racemanList = GfDirGetListFiltered("data/config/raceman", "xml");
	if (!racemanList) {
...
ReInitCars(void)
{
	...
	for (i = 1; i < nCars + 1; i++) {
		...
		//fix
		extern int berniw(tModInfo *modInfo);

		//ReInfo->modList = new tModList[20];

		//fix
		extern int InitFuncPtBerniw(int index, void *pt);

		extern int HumanInitFuncPt(int index, void *pt);

		//InitFuncPt(0, InitFuncPt);
		...
		for (j = 0; j < MAX_MOD_ITF; j++) 
		{
			...
			//if ((*(ReInfo->modList))->modInfo[j].index == robotIdx) 
			{
				...
				if (robotIdx == 1 ) // human
				{
					HumanInitFuncPt(robotIdx, (void*)(curRobot));
				}
				else
				{
					InitFuncPtBerniw(robotIdx, (void*)(curRobot));
				}
				//curModInfo->fctInit(robotIdx, (void*)(curRobot));
				snprintf(buf, BUFSIZE, "%sdata/drivers/%s/%s.xml", GetLocalDir(), cardllname, cardllname);
				...
				if (robhdle != NULL) {
					...
					snprintf(buf, BUFSIZE, "%sdata/cars/%s/%s.xml", GetLocalDir(), elt->_carName, elt->_carName);
					...
					snprintf(buf, BUFSIZE, "Loading Driver %-20s... Car: %s", "CAR", elt->_carName);
					//fix
					...
					if (category != 0) {
						...
						snprintf(buf, BUFSIZE, "%sdata/categories/%s.xml", GetLocalDir(), category);
...
ReInitTrack(void)
{
	...
	RmLoadingScreenSetText(buf);
	snprintf(buf, BUFSIZE, "%sdata/tracks/%s/%s/%s.%s", GetLocalDir(), catName, trackName, trackName, TRKEXT);

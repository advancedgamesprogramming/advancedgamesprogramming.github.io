...
GenCarsInfo(void)
{
	...
	files = GfDirGetList("data/categories");
	curFile = files;
	if ((curFile != NULL) && (curFile->name[0] != '.')) {
		do {
			...
			*str = '\0';
			...
			snprintf(buf, BUFSIZE, "%s/data/categories/%s.xml", GetLocalDir(), curFile->name);
	...
	if ((curFile != NULL) && (curFile->name[0] != '.')) {
		do {
			...
			snprintf(buf, BUFSIZE, "%s/data/cars/%s/%s.xml", GetLocalDir(), curFile->name, curFile->name);

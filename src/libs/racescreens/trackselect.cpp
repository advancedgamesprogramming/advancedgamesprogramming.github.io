...
/***************************************************************************
    ...
    email                : torcs@free.fr
    version              : $Id: trackselect.cpp,v 1.5.2.6 2014/02/10 18:31:00 berniw Exp $
 ***************************************************************************/
...
{
	snprintf(buf, BUFSIZE, "data/tracks/%s/%s/%s.png", CategoryList->name,
...
static void rmUpdateTrackInfo(void)
{
	...
	snprintf(buf, BUFSIZE, "data/tracks/%s/%s/%s.%s", CategoryList->name, ((tFList*)CategoryList->userData)->name,
...
RmGetTrackName(char *category, char *trackName)
{
	...
	snprintf(buf, BUFSIZE, "data/tracks/%s/%s/%s.%s", category, trackName, trackName, TRKEXT);
...
RmTrackSelect(void *vs)
{
	...
	CategoryList = GfDirGetList("data/tracks");
	...
	do {
		...
		snprintf(buf, BUFSIZE, "data/tracks/%s", CatCur->name);
